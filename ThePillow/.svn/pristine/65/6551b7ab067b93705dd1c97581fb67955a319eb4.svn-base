...
- (id)initWithFrame:(CGRect)frame
{
    if (self = [super initWithFrame:frame]) {
        self.pullToRefreshText = NSLocalizedString(@"xlistview_header_hint_normal", nil);
        self.releaseToRefreshText = NSLocalizedString(@"xlistview_header_hint_ready", nil);
        self.refreshingText = NSLocalizedString(@"xlistview_header_hint_loading", nil);
    }
    return self;
}
...
//    } else
    ...
    // 3.显示日期
    self.lastUpdateTimeLabel.text = [NSString stringWithFormat:@"%@%@",NSLocalizedString(@"xlistview_header_last_time", nil), time];
}
