...
#define INDICATORCOLOR MainColor.CGColor // 中间指示器颜色
...
//        self.backgroundColor = [UIColor whiteColor];
        ...
        rulerScrollView.rulerMin = _rulerMin;
        rulerScrollView.isText = _isText;
        rulerScrollView.isRangingHeight = _isRangingHeight;
        rulerScrollView.distanceTopAndBottom = _distanceTopAndBottom;
...
- (void)scrollViewDidScroll:(AHRulerScrollView *)scrollView {
    ...
    if (self.rulerDeletate) {
        ...
        scrollView.mode = NO;
        [self.rulerDeletate ahRuler:scrollView andRulerView:self];
    }
}
...
//    [self.layer addSublayer:gradient];
    ...
    shapeLayerLine.strokeColor = MainColor.CGColor;
    shapeLayerLine.fillColor = INDICATORCOLOR;
    shapeLayerLine.lineWidth = 1.f;
    shapeLayerLine.lineCap = kCALineCapSquare;
    
    NSUInteger ruleHeight = 20+25; // 文字高度
    CGMutablePathRef pathLine = CGPathCreateMutable();
    if (_isRangingHeight) {
        CGPathMoveToPoint(pathLine, NULL, self.frame.size.width / 2, self.frame.size.height - _distanceTopAndBottom - ruleHeight+5);
    }else {
        
        CGPathMoveToPoint(pathLine, NULL, self.frame.size.width / 2, self.frame.size.height - _distanceTopAndBottom - ruleHeight+25+5);
    }
    CGPathAddLineToPoint(pathLine, NULL, self.frame.size.width / 2, _distanceTopAndBottom + SHEIGHT);
    CGPathAddLineToPoint(pathLine, NULL, self.frame.size.width / 2 - SHEIGHT / 2, _distanceTopAndBottom);
    CGPathAddLineToPoint(pathLine, NULL, self.frame.size.width / 2 + SHEIGHT / 2, _distanceTopAndBottom);
    CGPathAddLineToPoint(pathLine, NULL, self.frame.size.width / 2, _distanceTopAndBottom + SHEIGHT);
